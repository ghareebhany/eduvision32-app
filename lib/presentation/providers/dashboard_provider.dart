...
class DashboardStats {
  ...
  factory DashboardStats.fromJson(Map<String, dynamic> j) => DashboardStats(
        ...
        inProgress: (j['in_progress'] as List? ?? [])
            ...
            .toList(),
        quizCount:       (j['hub']?['quizzes'])     as int? ?? 0,
        questionCount:   (j['hub']?['questions'])   as int? ?? 0,
        assignmentCount: (j['hub']?['assignments']) as int? ?? 0,
        reviewCount:     (j['hub']?['reviews'])     as int? ?? 0,
      );
}
